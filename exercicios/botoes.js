...
const botoes = Extra.markup(Markup.inlineKeyboard([
    ...
    Markup.callbackButton('🔄Resetar', 'reset'),
    Markup.callbackButton('Resultado', 'result')
], { columns: 3}))

bot.start(async ctx => {
    const from = ctx.update.message.from.first_name
    await ctx.reply(`Seja bem vindo, ${from}!`)
    await ctx.reply(`A contagem actual está em ${contagem}`, botoes)
})

bot.action(/add (\d+)/, ctx => {
    contagem += parseInt(ctx.match[1])
    ctx.reply(`A contagem actual está em ${contagem}`, botoes)
...
bot.action('result', ctx => {
    ctx.answerCbQuery(`A contagem actual está em ${contagem}`)
})
